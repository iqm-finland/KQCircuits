...
class TestStructure(Element):
    ...
    def produce_four_point_pads(self, pads_region, pad_width, pad_height, pad_spacing_x, pad_spacing_y, connect_pads,
        ...
        if refpoint_distance is None:
            ...
            self.refpoints[f"{refpoint_prefix}_sw"] = pos_sw + pya.DVector(
                -pad_width / 2 + refpoint_distance, 0
            )
            self.refpoints[f"{refpoint_prefix}_nw"] = pos_nw + pya.DVector(
                -pad_width / 2 + refpoint_distance, 0
            )
            self.refpoints[f"{refpoint_prefix}_ne"] = pos_ne + pya.DVector(
                pad_width / 2 - refpoint_distance, 0
            )
            self.refpoints[f"{refpoint_prefix}_se"] = pos_se + pya.DVector(
                pad_width / 2 - refpoint_distance, 0
            )
