...
def rational_fit(f, s_data, num_order, denom_order):
    ...
    if has_polyrat:
        ...
        try:
            ...
            logging.warning("Least squares fit failed in interpolated frequency sweep, "
...
def sweep_orders_and_fit(f_all,
                         s_all,
                         min_num_order=0,
                         max_num_order=10,
                         min_denom_order=2,
...
def interpolating_frequency_sweep(json_data,
    ...
    """
    if not has_polyrat:
        logging.warning("Rational fit using scipy.curve_fit is extremely unreliable."
    ...
    def _sample_on_slope(func, f_sampled, s_sampled, batch_size, max_p_factor=5, nevals=10000):
        ...
        for ind in range(batch_size):
            choices[ind] = separate_sample(choices[ind],
    ...
    while s_error > max_delta_s and iteration_count < max_iter:
        if iteration_count == 1:
            ...
            cur_freqs = _sample_on_slope(prev_func_mag,
                                         f_all,
                                         s_mag_fit,
                                         frequency_batch)
        ...
        # run elmer
        _run_elmer_solver(sim_name=simname,
                          sif_names=sif_names,
