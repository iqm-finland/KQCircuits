...
def get_data_extraction_sif(
    ...
    if dim != len(points_list[0]):
        print("Sampled coordinate dimensions and json dimensions do not match")
        sys.exit()
...
def get_elmer_results(path: str | Path, tmp_results_file: str):
    ...
    nkeys = len(data_keys)
    if ncols % nkeys != 0:
        print(f"Warning: number of columns is not divisible by the number of unique keys found in {tmp_results_file}")
    if nrows > 1:
...
if not tls_files:
    print("No sample point files ending with `_tls_mc.json` found.")
    sys.exit()
...
for tls_file in tls_files:
    ...
    if next(Path(sim_folder).glob(f"{elmer_data_file}*"), None) is None:
        print(f'Elmer model data "{elmer_data_file}" not found')
        print("Make sure the simulation is exported and run with the solution option `save_elmer_data=True`")
        continue
