...
Extracts fields for all coordinates collected in a .json file with a name like `*_tls_<filename>.json`, where
<filename> is specified by the argument ``--filename``.
These files can be generated by running `tls_monte_carlo_points.py` or `tls_metal_edge_points.py` before this script.
...
parser.add_argument(
    "--filename", type=str, default="mc", help="Optional: Whether to use Monte Carlo or metal edge sampling"
)

args = parser.parse_args()

tls_files = list(Path("").glob(f"*_tls_{args.filename}.json"))
if not tls_files:
    logging.warning(f"No sample point files ending with `_tls_{args.filename}.json` found.")
    sys.exit()

for tls_file in tls_files:
    def_file = str(tls_file).removesuffix(f"_tls_{args.filename}.json") + ".json"
    ...
    final_result_filename = f"{sim_folder}_fields_{args.filename}.csv"
    df_list = []
