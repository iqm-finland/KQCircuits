...
if result_files:
    ...
    for key, result_file in zip(parameter_values.keys(), result_files):
        ...
        if "groups" not in pp_data:
            ...
            epr_dict[key] = {
                "p_" + group: sum([v / total_energy for k, v in energy.items() if group in k])
            ...
            if not os.path.isfile(full_mer_path):
                ...
                if not os.path.isfile(full_mer_path):
                    full_mer_path = os.path.join(
                        os.path.abspath(__file__ + "/../../../"), "/".join(full_mer_path.split("/")[-2:])
                    )
            ...
            for group in pp_data["groups"]:
                ...
                epr_dict[key]["p_" + group + "_fixed"] = (
                    epr_dict[key]["p_" + group + "_mer_fixed"] + epr_dict[key]["p_" + group + "_nonmer"]
                )
