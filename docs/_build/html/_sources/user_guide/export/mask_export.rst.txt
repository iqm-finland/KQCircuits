...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    #. Add the following script to the ``kqcircuits_scripts/masks`` folder in
    ...
    #. (Optional) Open ``kqcircuits_scripts/macros/logging_setup`` and run it.
