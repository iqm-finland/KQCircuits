...
def test_decodes_standard_python_types():
    ...
    assert json.loads("[42, 1.23, \"test_string\", true, false, null]", cls=GeometryJsonDecoder) == list_var,\
...
def test_decodes_standard_dicts():
    ...
    assert set(json_var["object"].keys()) == set(
        ["inner_integer", "inner_number", "inner_string", "inner_bool_true",
         "inner_bool_false", "inner_null_field", "inner_list"]
    ), "Object not decoded correctly"
...
def test_decodes_dbox():
    ...
    assert json_dbox.left == -1000 and json_dbox.bottom == 5.67 and\
        json_dbox.right == 0 and json_dbox.top == 1234.56, "DBox not decoded correctly"
    ...
    assert decoded_dbox.left == -1000 and decoded_dbox.bottom == 5.67 and\
        decoded_dbox.right == 0 and decoded_dbox.top == 1234.56, "DBox not decoded correctly"
...
def test_decodes_dpath():
    json_dpath = json.loads("""{
        ...
        ], "width": 13}""", cls=GeometryJsonDecoder)
...
def test_decodes_dedge():
    ...
    assert json_dedge.x1 == -1000 and json_dedge.y1 == 5.67 and\
        json_dedge.x2 == 0 and json_dedge.y2 == 1234.56, "DEdge not decoded correctly"
    ...
    assert decoded_dedge.x1 == -1000 and decoded_dedge.y1 == 5.67 and\
        decoded_dedge.x2 == 0 and decoded_dedge.y2 == 1234.56, "DEdge not decoded correctly"
...
def test_decode_composite_dict():
    ...
    assert test_json["test_layer"].layer == 1001 and test_json["test_layer"].datatype == 2,\
        "Dict with pya shapes not decoded correctly"
    ...
    assert test_json["test_vectors"][0].x == 1.2 and test_json["test_vectors"][0].y == 3,\
        "Dict with pya shapes not decoded correctly"
    assert isinstance(test_json["test_vectors"][1], pya.DVector), "Dict with pya shapes not decoded correctly"
    assert test_json["test_vectors"][1].x == -3.4 and test_json["test_vectors"][1].y == 5,\
        "Dict with pya shapes not decoded correctly"
    ...
    assert test_json["test_vectors"][3].x == -1000 and test_json["test_vectors"][3].y == 1000,\
        "Dict with pya shapes not decoded correctly"

    assert isinstance(test_json["test_point_sets"], dict), "Dict with pya shapes not decoded correctly"
    assert set(test_json["test_point_sets"].keys()) == set(["path", "polygon", "just_points"]),\
        "Dict with pya shapes not decoded correctly"
    ...
    assert isinstance(test_json["test_point_sets"]["polygon"], pya.DPolygon),\
        "Dict with pya shapes not decoded correctly"
    ...
    _check_dpoint_obj(test_json["test_point_sets"]["just_points"][0], -2, 2,
        "Dict with pya shapes not decoded correctly")
    _check_dpoint_obj(test_json["test_point_sets"]["just_points"][1], 1, -1,
        "Dict with pya shapes not decoded correctly")
    _check_dpoint_obj(test_json["test_point_sets"]["just_points"][2], 1.33, 2.66,
        "Dict with pya shapes not decoded correctly")
    ...
    assert set(test_json["deep_hierarchy"].keys()) == set(["very_deep_hierarchy", "box"]),\
        "Dict with pya shapes not decoded correctly"
    assert isinstance(test_json["deep_hierarchy"]["very_deep_hierarchy"], dict),\
        "Dict with pya shapes not decoded correctly"
    assert set(test_json["deep_hierarchy"]["very_deep_hierarchy"].keys()) == set(["edge"]),\
        "Dict with pya shapes not decoded correctly"
    assert isinstance(test_json["deep_hierarchy"]["very_deep_hierarchy"]["edge"], pya.DEdge),\
        "Dict with pya shapes not decoded correctly"
    assert  test_json["deep_hierarchy"]["very_deep_hierarchy"]["edge"].x1 == -123.45 and\
            test_json["deep_hierarchy"]["very_deep_hierarchy"]["edge"].y1 == 54.321 and\
            test_json["deep_hierarchy"]["very_deep_hierarchy"]["edge"].x2 == 13.421 and\
            test_json["deep_hierarchy"]["very_deep_hierarchy"]["edge"].y2 == 45.231,\
        "Dict with pya shapes not decoded correctly"
    assert isinstance(test_json["deep_hierarchy"]["box"], pya.DBox),\
        "Dict with pya shapes not decoded correctly"
    assert  test_json["deep_hierarchy"]["box"].left == -98.765 and\
            test_json["deep_hierarchy"]["box"].bottom == -89.763 and\
            test_json["deep_hierarchy"]["box"].right == 67.876 and\
            test_json["deep_hierarchy"]["box"].top == 789.87,\
        "Dict with pya shapes not decoded correctly"
    ...
    assert test_dict["test_layer"].layer == 1001 and test_dict["test_layer"].datatype == 2,\
        "Dict with pya shapes not decoded correctly"
    ...
    assert test_dict["test_vectors"][0].x == 1.2 and test_dict["test_vectors"][0].y == 3,\
        "Dict with pya shapes not decoded correctly"
    assert isinstance(test_dict["test_vectors"][1], pya.DVector), "Dict with pya shapes not decoded correctly"
    assert test_dict["test_vectors"][1].x == -3.4 and test_dict["test_vectors"][1].y == 5,\
        "Dict with pya shapes not decoded correctly"
    ...
    assert test_dict["test_vectors"][3].x == -1000 and test_dict["test_vectors"][3].y == 1000,\
        "Dict with pya shapes not decoded correctly"

    assert isinstance(test_dict["test_point_sets"], dict), "Dict with pya shapes not decoded correctly"
    assert set(test_dict["test_point_sets"].keys()) == set(["path", "polygon", "just_points"]),\
        "Dict with pya shapes not decoded correctly"
    ...
    assert isinstance(test_dict["test_point_sets"]["polygon"], pya.DPolygon),\
        "Dict with pya shapes not decoded correctly"
    ...
    _check_dpoint_obj(test_dict["test_point_sets"]["just_points"][0], -2, 2,
        "Dict with pya shapes not decoded correctly")
    _check_dpoint_obj(test_dict["test_point_sets"]["just_points"][1], 1, -1,
        "Dict with pya shapes not decoded correctly")
    _check_dpoint_obj(test_dict["test_point_sets"]["just_points"][2], 1.33, 2.66,
        "Dict with pya shapes not decoded correctly")
    ...
    assert set(test_dict["deep_hierarchy"].keys()) == set(["very_deep_hierarchy", "box"]),\
        "Dict with pya shapes not decoded correctly"
    assert isinstance(test_dict["deep_hierarchy"]["very_deep_hierarchy"], dict),\
        "Dict with pya shapes not decoded correctly"
    assert set(test_dict["deep_hierarchy"]["very_deep_hierarchy"].keys()) == set(["edge"]),\
        "Dict with pya shapes not decoded correctly"
    assert isinstance(test_dict["deep_hierarchy"]["very_deep_hierarchy"]["edge"], pya.DEdge),\
        "Dict with pya shapes not decoded correctly"
    assert  test_dict["deep_hierarchy"]["very_deep_hierarchy"]["edge"].x1 == -123.45 and\
            test_dict["deep_hierarchy"]["very_deep_hierarchy"]["edge"].y1 == 54.321 and\
            test_dict["deep_hierarchy"]["very_deep_hierarchy"]["edge"].x2 == 13.421 and\
            test_dict["deep_hierarchy"]["very_deep_hierarchy"]["edge"].y2 == 45.231,\
        "Dict with pya shapes not decoded correctly"
    assert isinstance(test_dict["deep_hierarchy"]["box"], pya.DBox),\
        "Dict with pya shapes not decoded correctly"
    assert  test_dict["deep_hierarchy"]["box"].left == -98.765 and\
            test_dict["deep_hierarchy"]["box"].bottom == -89.763 and\
            test_dict["deep_hierarchy"]["box"].right == 67.876 and\
            test_dict["deep_hierarchy"]["box"].top == 789.87,\
        "Dict with pya shapes not decoded correctly"
