...
from kqcircuits.util.geometry_json_encoder import GeometryJsonEncoder, GeometryJsonDecoder,\
...
@pytest.mark.parametrize("parameters", [
    ...
    pya.DPolygon(pya.DBox(-100, -100, 100, 100)).insert_hole(pya.DBox(-50, -50, -10, -10))
...
def test_no_data_loss(parameters):
    assert parameters == json.loads(json.dumps(parameters, cls=GeometryJsonEncoder), cls=GeometryJsonDecoder),\
        "Original instance not equal to encoded then decoded instance"
    assert parameters == decode_dict_as_python_obj(encode_python_obj_as_dict(parameters)),\
        "Original instance not equal to encoded then decoded instance"
