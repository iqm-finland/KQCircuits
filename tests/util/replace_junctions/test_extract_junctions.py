...
def check_static_cell_extracts_same_junctions(junctions, static_cell, caplog):
    ...
    assert sorted(junctions, key=comparator) == sorted(static_junctions, key=comparator), \
        "Static cell produces different set of junction entries than PCell"
    log_messages = [x.message for x in caplog.records]
    assert len(log_messages) == 1 and log_messages[0] == "Top cell doesn't contain PCell parameter data", \
...
def test_junction_entry_equality():
    ...
    assert junction_entry == JunctionEntry(Manhattan, t1_, {"b":6, "a":10}, "qb_0", "squid"), \
        "Expected these JunctionEntry objects to be considered equal"
    assert junction_entry != JunctionEntry(ManhattanSingleJunction, t1, {"a":10, "b":6}, "qb_0", "squid"), \
        "JunctionEntry objects should not be considered equal if they have different type"
    assert junction_entry != JunctionEntry(type(None), t1, {"a":10, "b":6}, "qb_0", "squid"), \
        "JunctionEntry objects should not be considered equal if they have different type"
    assert junction_entry != JunctionEntry(Manhattan, t2, {"a":10, "b":6}, "qb_0", "squid"), \
        "JunctionEntry objects should not be considered equal if they have different transformation"
    assert junction_entry != JunctionEntry(Manhattan, t1, {"a":10, "b":7}, "qb_0", "squid"), \
        "JunctionEntry objects should not be considered equal if they have different parameters"
    assert junction_entry != JunctionEntry(Manhattan, t1, {"a":10, "b":6, "c":0}, "qb_0", "squid"), \
        "JunctionEntry objects should not be considered equal if they have different parameters"
    assert junction_entry != JunctionEntry(Manhattan, t1, {"b":6}, "qb_0", "squid"), \
        "JunctionEntry objects should not be considered equal if they have different parameters"
    assert junction_entry != JunctionEntry(Manhattan, t1, {"a":10, "b":6}, "qb_1", "squid"), \
        "JunctionEntry objects should not be considered equal if they have different parent name"
    assert junction_entry != JunctionEntry(Manhattan, t1, {"a":10, "b":6}, "qb_0", "squid_1"), \
        "JunctionEntry objects should not be considered equal if they have different parent name"
...
def test_junctions_get_extracted_from_pcell(pcell):
    ...
    assert all(vertical_test_array_params[0] == param for param in vertical_test_array_params), \
        "Vertical test array junctions should have same junction parameters"
    assert all(horizontal_test_array_params[0] == param for param in horizontal_test_array_params), \
        "Horizontal test array junctions should have same junction parameters"
    assert all(qb_params[0] == param for param in qb_params), \
        "Qubit junctions should have same junction parameters"
...
def test_junction_parameters_must_be_exhaustive_for_static_cell1(static_cell):
    ...
    assert "'junction_type' value None for junction" in str(expected_error.value) and \
        "is not part of junction_type_choices" in str(expected_error.value), \
...
def test_junction_parameters_must_be_exhaustive_for_static_cell2(pcell, static_cell, caplog):
    ...
    tuned_params = {
        parent_name: {
            name: {
                "junction_type": params["junction_type"]
    ...
    assert "Some junction parameters were missing in the tuning json, see log for details" in \
        str(expected_error.value), f"Unexpected error {expected_error}"
    main_warning = [x.message for x in caplog.records][1]
    assert ("Since the cell doesn't contain pre-existing PCell parameter data, "
            "the tuned junction json should be exhaustive.\n"
            "Following junction parameters missing:\n\n"
            "Manhattan class junction parameters missing {") in main_warning and \
            all(x in main_warning for x in ["'face_ids'", "'junction_width'", "'loop_area'"]), \
...
def test_junction_cant_be_none_for_static_cell(pcell, static_cell):
    ...
    with pytest.raises(ValueError) as expected_error:
        extract_junctions(static_cell, tuned_params)
    assert "'junction_type' value None for junction (qb_1, squid) is not part of junction_type_choices" \
        in str(expected_error.value), f"Unexpected error {expected_error}"
...
def test_tune_parameter1(pcell, static_cell, caplog):
    ...
    assert [j.parameters for j in junctions if
            j.parent_name == "qb_1" and j.name == "squid"][0]["junction_width"] == 2.0, \
        "Expected junction_width for qb_1 to be tuned to 2.0"
    ...
    assert all(j.parameters["junction_width"] == other_junction_value for j in junctions if not (
        j.parent_name == "qb_1" and j.name == "squid")), \
...
def test_tune_parameter2(pcell, static_cell, caplog):
    ...
    assert [j.parameters for j in junctions if
        ...
        "Expected junction_width for qb_1 to be tuned to 2.0"
    assert [j.parameters for j in junctions if
            j.parent_name == "qb_1" and j.name == "squid"][0]["loop_area"] == 50, \
        "Expected loop_area for qb_1 to be tuned to 50"
    junction_width_values = set(j.parameters["junction_width"] for j in junctions)
    other_junction_value = list(junction_width_values.difference({2.0}))[0]
    loop_area_values = set(j.parameters["loop_area"] for j in junctions)
    other_loop_value = list(loop_area_values.difference({50}))[0]
    assert all(j.parameters["junction_width"] == other_junction_value and \
               j.parameters["loop_area"] == other_loop_value for j in junctions if not (
        j.parent_name == "qb_1" and j.name == "squid")), \
        "All other junction_width values should be the same"
    check_static_cell_extracts_same_junctions(junctions, static_cell, caplog)
...
def test_tune_parameter3(pcell, static_cell, caplog):
    ...
    assert [j.parameters for j in junctions if
        ...
        "Expected loop_area for qb_1 to be tuned to 50"
    assert [j.parameters for j in junctions if
            j.parent_name == "testarray_s" and j.name == "squid_1"][0]["loop_area"] == 25, \
        "Expected loop_area for qb_1 to be tuned to 25"
    junction_width_values = set(j.parameters["junction_width"] for j in junctions)
    other_junction_value = list(junction_width_values.difference({2.0}))[0]
    loop_area_values = set(j.parameters["loop_area"] for j in junctions)
    other_loop_value = list(loop_area_values.difference({50, 25}))[0]
    assert all(j.parameters["junction_width"] == other_junction_value
               for j in junctions if not (j.parent_name == "qb_1" and j.name == "squid")), \
                "All other junction_width values should be the same"
    assert all(j.parameters["loop_area"] == other_loop_value
               for j in junctions if not (
                   (j.parent_name == "qb_1" and j.name == "squid") or \
                   (j.parent_name == "testarray_s" and j.name == "squid_1"))), \
                "All other junction_width values should be the same"
    check_static_cell_extracts_same_junctions(junctions, static_cell, caplog)
...
def test_change_junction_type(pcell, static_cell, caplog):
    ...
    for i,t1 in enumerate(transformations):
        for t2 in transformations[i+1:]:
            assert t1 != t2, "No pair of extracted junctions should have the same transformation"
    assert [j for j in junctions if j.parent_name == "qb_1"][0].type == ManhattanSingleJunction, \
         "Junction of qb_1 should be of type ManhattanSingleJunction"
    assert all(j.type == Manhattan for j in junctions if j.parent_name != "qb_1"), \
...
def test_changing_junction_type_requires_missing_parameters_for_pcell1(pcell, caplog):
    ...
    assert ("Since junction type was changed for some junctions, "
            ...
            "ManhattanSingleJunction class junction parameters missing {") in main_warning and \
            all(x in main_warning for x in
                ["'pad_rounding_radius'", "'include_base_metal_addition'",
                 "'height'", "'width'", "'pad_to_pad_separation'",
                 "'x_offset'", "'pad_width'", "'pad_height'"]) and \
            "missing for [('qb_1', 'squid')]\n\n" in main_warning
...
def test_changing_junction_type_for_pcell_can_reuse_parameter_values(pcell):
    ...
    assert all(j.parameters["offset_compensation"] == 0.0 for j in junctions2), \
...
def test_changing_junction_type_expects_different_schema_for_static_cell(pcell, static_cell, caplog):
    ...
    params_with_changed_junction = extract_junctions(pcell, {
        "qb_1": {"squid": {
            "junction_type": "Manhattan Single Junction",
            # Parameters that ManhattanSingleJunction uses but Manhattan does not use
            "width": 22,
...
def test_warn_about_surplus_parameters_pcell1(pcell, caplog):
    ...
    assert ("Manhattan class junction attempted to be tuned with parameters "
            "that are not part of the class: {'foobar'}") in warnings[0], f"Unexpected warning: {warnings[0]}"
...
def test_warn_about_surplus_parameters_pcell2(pcell, caplog):
    ...
    warnings = [x.message for x in caplog.records]
    assert ("Manhattan class junction attempted to be tuned with parameters "
            "that are not part of the class: {") in warnings[0] and \
            all(x in warnings[0] for x in
                ["'width'", "'pad_to_pad_separation'", "'pad_height'", "'pad_width'",
                 "'include_base_metal_addition'", "'x_offset'", "'height'", "'pad_rounding_radius'", ]), \
...
def test_warn_about_surplus_parameters_static1(pcell, static_cell, caplog):
    ...
    assert ("Manhattan class junction attempted to be tuned with parameters "
            "that are not part of the class: {'foobar'}") in warnings[1], f"Unexpected warning: {warnings[1]}"
...
def test_warn_about_surplus_parameters_static2(pcell, static_cell, caplog):
    ...
    assert "Top cell doesn't contain PCell parameter data" in warnings[0], f"Unexpected warning: {warnings[0]}"
    assert ("Manhattan class junction attempted to be tuned with parameters "
            "that are not part of the class: {") in warnings[1] and \
            all(x in warnings[1] for x in
                ["'width'", "'pad_to_pad_separation'", "'pad_height'", "'pad_width'",
                 "'include_base_metal_addition'", "'x_offset'", "'height'", "'pad_rounding_radius'", ]), \
...
def test_cant_change_to_junction_type_that_doesnt_exist(pcell):
    ...
    assert "'junction_type' value Foobar Junction for junction (qb_1, squid) is not part of junction_type_choices" in \
        str(expected_error.value), f"Unexpected error {expected_error}"
...
def test_recreating_chip_with_no_tuning_has_identical_geometry(pcell):
    ...
    for layer_info, junction_region in junction_regions.items():
        ...
        if layer_info.name in must_be_exact:
            assert (region ^ junction_region).is_empty(), \
                f"In layer {layer_info.name} junction geometry should be a identical to original geometry"
        elif layer_info.name in must_be_within:
            assert (junction_region - region).is_empty(), \
                f"In layer {layer_info.name} junction geometry should be a subset of original geometry"
...
def test_recreating_chip_with_some_tuning_has_limited_diff_in_geometry1(pcell):
    ...
    for layer_info, junction_region in junction_regions.items():
        ...
        if layer_info.name in must_be_exact:
            ...
            assert (diff_region - test_box).is_empty(), \
                f"All geometry diffs at layer {layer_info.name} should be contained within a bounding box"
        elif layer_info.name in must_be_within:
            diff_region = junction_region - region
            assert (diff_region - test_box).is_empty(), \
                f"All geometry diffs at layer {layer_info.name} should be contained within a bounding box"
    assert has_diff_somewhere, ("There was no difference in geometry between original and junction cell, "
